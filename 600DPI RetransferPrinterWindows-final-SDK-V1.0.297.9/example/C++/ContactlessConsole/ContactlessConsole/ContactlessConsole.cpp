// ContactlessConsole.cpp : 定义控制台应用程序的入口点。
//
...
Step1RWCard_Step2Print:
	...
	RE = R600CardInject(5); //进卡到非接读卡器非接位
	if(RE) { return outputError(RE); }

	//读写卡
	switch(RWConfig.CardType)
	{
	case 1:
		ReaderCPURW();
		break;
...
Step1Print_Step2RWCard:
	...
	std::cout << "移卡到非接读卡器非接位" << std::endl;
	RE = R600CardMove(5); //移卡
	if(RE) { return outputError(RE); }
